# Jags model for Nest survival using "fixed effects" only
#
# 2019-06-28 CJS First edition
#
...
jags.nest.survival.fixed.effects <- function(
         nestdata,   # nest data
...
# Input are 2 data frames which have the nest data  and the nest, time, nest x time covariates.
...
init.list <- list(
  list(beta=rep(0, ncol(fe.design))),
  list(beta=rep(0, ncol(fe.design))),
  list(beta=rep(0, ncol(fe.design)))
)
